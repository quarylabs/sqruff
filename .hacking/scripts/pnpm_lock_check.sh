...
#
# Runs `pnpm install --frozen-lockfile --lockfile-only --trust-lockfile`, which
# fails when the specifiers in pnpm-lock.yaml no longer match the package.json
# manifests (for example after a dependency bump that forgot to regenerate the
# lockfile). Trusting the committed lockfile avoids pnpm's network-dependent
# supply-chain verification; that is separate from this consistency check.
#
...
if ! "$PNPM_BIN" --dir "$WORKDIR" install --frozen-lockfile --lockfile-only --trust-lockfile; then
    echo ""
