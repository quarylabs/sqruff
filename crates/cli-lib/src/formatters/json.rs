...
use sqruff_lib::{Formatter, api::LintDiagnostic, core::linter::linted_file::LintedFile};
...
impl JsonFormatter {
    pub(crate) fn dispatch_file_diagnostics(&self, fname: &str, diagnostics: &[LintDiagnostic]) {
        let mut lock = self.violations.lock().unwrap();
        lock.entry(fname.into())
            .or_default()
            .extend(diagnostics.iter().map(Diagnostic::from).collect::<Vec<_>>());
    }
