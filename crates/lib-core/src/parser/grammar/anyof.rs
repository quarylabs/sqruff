...
pub fn simple(
    ...
    let option_simples: Vec<Option<(FxHashSet<String>, SyntaxSet)>> = elements
        .iter()
    ...
    let simple_buff: Vec<(FxHashSet<String>, SyntaxSet)> =
        option_simples.into_iter().flatten().collect();

    let simple_raws: FxHashSet<_> = simple_buff
        .iter()
