//! T-SQL keyword classifications synchronized with SQLFluff.

use hashbrown::HashSet;

/// Current T-SQL reserved keywords from Microsoft documentation.
pub(crate) fn tsql_reserved_keywords() -> HashSet<&'static str> {
    [
        "ADD",
        "ALL",
        "ALTER",
        "AND",
        "ANY",
        "AS",
        "ASC",
        "AUTHORIZATION",
        "BACKUP",
        "BEGIN",
        "BETWEEN",
        "BREAK",
        "BROWSE",
        "BULK",
        "BY",
        "CASCADE",
        "CASE",
        "CHECK",
        "CHECKPOINT",
        "CLOSE",
        "CLUSTERED",
        "COALESCE",
        "COLLATE",
        "COLUMN",
        "COMMIT",
        "COMPUTE",
        "CONSTRAINT",
        "CONTAINS",
        "CONTAINSTABLE",
        "CONTINUE",
        "CONVERT",
        "CREATE",
        "CROSS",
        "CURRENT",
        "CURRENT_DATE",
        "CURRENT_TIME",
        "CURRENT_TIMESTAMP",
        "CURRENT_USER",
        "CURSOR",
        "DATABASE",
        "DBCC",
        "DEALLOCATE",
        "DECLARE",
        "DEFAULT",
        "DELETE",
        "DENY",
        "DESC",
        "DISK",
        "DISTINCT",
        "DISTRIBUTED",
        "DOUBLE",
        "DROP",
        "DUMP",
        "ELSE",
        "END",
        "ERRLVL",
        "ESCAPE",
        "EXCEPT",
        "EXEC",
        "EXECUTE",
        "EXISTS",
        "EXIT",
        "EXTERNAL",
        "FETCH",
        "FILE",
        "FILLFACTOR",
        "FOR",
        "FOREIGN",
        "FREETEXT",
        "FREETEXTTABLE",
        "FROM",
        "FULL",
        "FUNCTION",
        "GO",
        "GOTO",
        "GRANT",
        "GROUP",
        "HAVING",
        "HOLDLOCK",
        "IDENTITY",
        "IDENTITY_INSERT",
        "IDENTITYCOL",
        "IF",
        "IN",
        "INDEX",
        "INNER",
        "INSERT",
        "INTERSECT",
        "INTO",
        "IS",
        "JOIN",
        "KEY",
        "KILL",
        "LEFT",
        "LIKE",
        "LINENO",
        "LOAD",
        "MERGE",
        "NATIONAL",
        "NOCHECK",
        "NONCLUSTERED",
        "NOT",
        "NULL",
        "NULLIF",
        "OF",
        "OFF",
        "OFFSETS",
        "ON",
        "OPEN",
        "OPENDATASOURCE",
        "OPENQUERY",
        "OPENROWSET",
        "OPENXML",
        "OPTION",
        "OR",
        "ORDER",
        "OUTER",
        "OVER",
        "PERCENT",
        "PIVOT",
        "PLAN",
        "PRECISION",
        "PRIMARY",
        "PRINT",
        "PROC",
        "PROCEDURE",
        "PUBLIC",
        "RAISERROR",
        "READ",
        "READTEXT",
        "RECONFIGURE",
        "REFERENCES",
        "REPLICATION",
        "RESTORE",
        "RESTRICT",
        "RETURN",
        "REVERT",
        "REVOKE",
        "RIGHT",
        "ROLLBACK",
        "ROWCOUNT",
        "ROWGUIDCOL",
        "RULE",
        "SAVE",
        "SCHEMA",
        "SECURITYAUDIT",
        "SELECT",
        "SEMANTICKEYPHRASETABLE",
        "SEMANTICSIMILARITYDETAILSTABLE",
        "SEMANTICSIMILARITYTABLE",
        "SESSION_USER",
        "SET",
        "SETUSER",
        "SHUTDOWN",
        "SOME",
        "STATISTICS",
        "SYSTEM_USER",
        "TABLE",
        "TABLESAMPLE",
        "TEXTSIZE",
        "THEN",
        "TO",
        "TOP",
        "TRAN",
        "TRANSACTION",
        "TRIGGER",
        "TRUNCATE",
        "TRY_CONVERT",
        "TSEQUAL",
        "UNION",
        "UNIQUE",
        "UNPIVOT",
        "UPDATE",
        "UPDATETEXT",
        "USE",
        "USER",
        "VALUES",
        "VARYING",
        "VIEW",
        "WAITFOR",
        "WHEN",
        "WHERE",
        "WHILE",
        "WITH",
        "WITHIN GROUP",
        "WRITETEXT",
    ]
    .into_iter()
    .collect()
}

/// Future-reserved T-SQL keywords, which remain valid unquoted identifiers.
pub(crate) fn tsql_future_keywords() -> HashSet<&'static str> {
    [
        "ABSOLUTE",
        "ACTION",
        "ADMIN",
        "AFTER",
        "AGGREGATE",
        "ALIAS",
        "ALLOCATE",
        "ARE",
        "ARRAY",
        "ASENSITIVE",
        "ASSERTION",
        "ASYMMETRIC",
        "AT",
        "ATOMIC",
        "BEFORE",
        "BINARY",
        "BIT",
        "BLOB",
        "BOOLEAN",
        "BOTH",
        "BREADTH",
        "CALL",
        "CALLED",
        "CARDINALITY",
        "CASCADED",
        "CAST",
        "CATALOG",
        "CHAR",
        "CHARACTER",
        "CLASS",
        "CLOB",
        "COLLATION",
        "COLLECT",
        "COMPLETION",
        "CONDITION",
        "CONNECT",
        "CONNECTION",
        "CONSTRAINTS",
        "CONSTRUCTOR",
        "CORR",
        "CORRESPONDING",
        "COVAR_POP",
        "COVAR_SAMP",
        "CUBE",
        "CUME_DIST",
        "CURRENT_CATALOG",
        "CURRENT_DEFAULT_TRANSFORM_GROUP",
        "CURRENT_PATH",
        "CURRENT_ROLE",
        "CURRENT_SCHEMA",
        "CURRENT_TRANSFORM_GROUP_FOR_TYPE",
        "CYCLE",
        "DATA",
        "DATE",
        "DAY",
        "DEC",
        "DECIMAL",
        "DEFERRABLE",
        "DEFERRED",
        "DEPTH",
        "DEREF",
        "DESCRIBE",
        "DESCRIPTOR",
        "DESTROY",
        "DESTRUCTOR",
        "DETERMINISTIC",
        "DIAGNOSTICS",
        "DICTIONARY",
        "DISCONNECT",
        "DOMAIN",
        "DYNAMIC",
        "EACH",
        "ELEMENT",
        "END-EXEC",
        "EQUALS",
        "EVERY",
        "EXCEPTION",
        "FALSE",
        "FILTER",
        "FIRST",
        "FLOAT",
        "FOUND",
        "FREE",
        "FULLTEXTTABLE",
        "FUSION",
        "GENERAL",
        "GET",
        "GLOBAL",
        "GROUPING",
        "HOLD",
        "HOST",
        "HOUR",
        "IGNORE",
        "IMMEDIATE",
        "INDICATOR",
        "INITIALIZE",
        "INITIALLY",
        "INOUT",
        "INPUT",
        "INT",
        "INTEGER",
        "INTERSECTION",
        "INTERVAL",
        "ISOLATION",
        "ITERATE",
        "LANGUAGE",
        "LARGE",
        "LAST",
        "LATERAL",
        "LEADING",
        "LESS",
        "LEVEL",
        "LIKE_REGEX",
        "LIMIT",
        "LN",
        "LOCAL",
        "LOCALTIME",
        "LOCALTIMESTAMP",
        "LOCATOR",
        "MAP",
        "MATCH",
        "MEMBER",
        "METHOD",
        "MINUTE",
        "MOD",
        "MODIFIES",
        "MODIFY",
        "MODULE",
        "MONTH",
        "MULTISET",
        "NAMES",
        "NATURAL",
        "NCHAR",
        "NCLOB",
        "NEW",
        "NEXT",
        "NO",
        "NONE",
        "NORMALIZE",
        "NUMERIC",
        "OBJECT",
        "OCCURRENCES_REGEX",
        "OLD",
        "ONLY",
        "OPERATION",
        "ORDINALITY",
        "OUT",
        "OUTPUT",
        "OVERLAY",
        "PAD",
        "PARAMETER",
        "PARAMETERS",
        "PARTIAL",
        "PARTITION",
        "PATH",
        "PERCENT_RANK",
        "PERCENTILE_CONT",
        "PERCENTILE_DISC",
        "POSITION_REGEX",
        "POSTFIX",
        "PREFIX",
        "PREORDER",
        "PREPARE",
        "PRESERVE",
        "PRIOR",
        "PRIVILEGES",
        "RANGE",
        "READS",
        "REAL",
        "RECURSIVE",
        "REF",
        "REFERENCING",
        "REGR_AVGX",
        "REGR_AVGY",
        "REGR_COUNT",
        "REGR_INTERCEPT",
        "REGR_R2",
        "REGR_SLOPE",
        "REGR_SXX",
        "REGR_SXY",
        "REGR_SYY",
        "RELATIVE",
        "RELEASE",
        "RESULT",
        "RETURNING",
        "RETURNS",
        "ROLE",
        "ROLLUP",
        "ROUTINE",
        "ROW",
        "ROWS",
        "SAVEPOINT",
        "SCOPE",
        "SCROLL",
        "SEARCH",
        "SECOND",
        "SECTION",
        "SENSITIVE",
        "SEQUENCE",
        "SESSION",
        "SETS",
        "SIMILAR",
        "SIZE",
        "SMALLINT",
        "SPACE",
        "SPECIFIC",
        "SPECIFICTYPE",
        "SQL",
        "SQLEXCEPTION",
        "SQLSTATE",
        "SQLWARNING",
        "START",
        "STATE",
        "STATEMENT",
        "STATIC",
        "STDDEV_POP",
        "STDDEV_SAMP",
        "STRUCTURE",
        "SUBMULTISET",
        "SUBSTRING_REGEX",
        "SYMMETRIC",
        "SYSTEM",
        "TEMPORARY",
        "TERMINATE",
        "THAN",
        "TIME",
        "TIMESTAMP",
        "TIMEZONE_HOUR",
        "TIMEZONE_MINUTE",
        "TRAILING",
        "TRANSLATE_REGEX",
        "TRANSLATION",
        "TREAT",
        "TRUE",
        "UESCAPE",
        "UNDER",
        "UNKNOWN",
        "UNNEST",
        "USAGE",
        "USING",
        "VALUE",
        "VAR_POP",
        "VAR_SAMP",
        "VARCHAR",
        "VARIABLE",
        "WHENEVER",
        "WIDTH_BUCKET",
        "WINDOW",
        "WITHIN",
        "WITHOUT",
        "WORK",
        "WRAPPER",
        "WRITE",
        "XMLAGG",
        "XMLATTRIBUTES",
        "XMLBINARY",
        "XMLCAST",
        "XMLCOMMENT",
        "XMLCONCAT",
        "XMLDOCUMENT",
        "XMLELEMENT",
        "XMLEXISTS",
        "XMLFOREST",
        "XMLITERATE",
        "XMLNAMESPACES",
        "XMLPARSE",
        "XMLPI",
        "XMLQUERY",
        "XMLSERIALIZE",
        "XMLTABLE",
        "XMLTEXT",
        "XMLVALIDATE",
        "YEAR",
        "ZONE",
    ]
    .into_iter()
    .collect()
}

/// T-SQL unreserved keywords.
pub(crate) fn tsql_unreserved_keywords() -> HashSet<&'static str> {
    [
        "ABORT_AFTER_WAIT",
        "ABORT",
        "ABSENT",
        "ACCELERATED_DATABASE_RECOVERY",
        "ACCENT_SENSITIVITY",
        "ADMINISTER",
        "ALGORITHM",
        "ALL_CONSTRAINTS",
        "ALL_ERRORMSGS",
        "ALL_INDEXES",
        "ALL_LEVELS",
        "ALLOW_CONNECTIONS",
        "ALLOW_ENCRYPTED_VALUE_MODIFICATIONS",
        "ALLOW_PAGE_LOCKS",
        "ALLOW_ROW_LOCKS",
        "ALLOW_SNAPSHOT_ISOLATION",
        "ALLOWED",
        "ALWAYS",
        "ANSI_DEFAULTS",
        "ANSI_NULL_DEFAULT",
        "ANSI_NULL_DFLT_OFF",
        "ANSI_NULL_DFLT_ON",
        "ANSI_NULLS",
        "ANSI_PADDING",
        "ANSI_WARNINGS",
        "APPEND_ONLY",
        "APPLY",
        "ARITHABORT",
        "ARITHIGNORE",
        "ASSEMBLY",
        "ATTACH",
        "ATTACH_REBUILD_LOG",
        "AUTHENTICATE",
        "AUTO_CLOSE",
        "AUTO_CREATE_STATISTICS",
        "AUTO_CREATE_TABLE",
        "AUTO_SHRINK",
        "AUTO_UPDATE_STATISTICS",
        "AUTO_UPDATE_STATISTICS_ASYNC",
        "AUTO",
        "BACKUP_CLONEDB",
        "BACKUP_STORAGE_REDUNDANCY",
        "BATCHSIZE",
        "BERNOULLI",
        "BLOCK",
        "BLOCKERS",
        "BLOCKSIZE",
        "BULK_LOGGED",
        "BUFFERCOUNT",
        "CACHE",
        "CALLER",
        "CATALOG_COLLATION",
        "CATCH",
        "CERTIFICATE",
        "CHANGE_TRACKING",
        "CHECK_CONSTRAINTS",
        "CHECK_EXPIRATION",
        "CHECK_POLICY",
        "CHECKALLOC",
        "CHECKSUM",
        "CHECKCATALOG",
        "CHECKCONSTRAINTS",
        "CHECKDB",
        "CHECKFILEGROUP",
        "CHECKIDENT",
        "CHECKTABLE",
        "CLEANTABLE",
        "CLONEDATABASE",
        "CODEPAGE",
        "COLLECTION",
        "COLUMN_ENCRYPTION_KEY",
        "COLUMNSTORE_ARCHIVE",
        "COLUMNSTORE",
        "COMMITTED",
        "COMPATIBILITY_LEVEL",
        "COMPRESS_ALL_ROW_GROUPS",
        "COMPRESSION_DELAY",
        "COMPRESSION",
        "CONCAT_NULL_YIELDS_NULL",
        "CONCAT",
        "CONTINUE_AFTER_ERROR",
        "CONNECTION_OPTIONS",
        "CONTAINED",
        "CONTENT",
        "COUNT_ROWS",
        "CONTAINMENT",
        "CONTEXT_INFO",
        "CONTROL",
        "COPY",
        "CREDENTIAL",
        "CURSOR_CLOSE_ON_COMMIT",
        "CURSOR_DEFAULT",
        "DATA_COMPRESSION",
        "DATA_CONSISTENCY_CHECK",
        "DATA_DELETION",
        "DATA_PURITY",
        "DATA_SOURCE",
        "DATABASE_DEFAULT",
        "DATAFILETYPE",
        "DATASOURCE",
        "DATE_CORRELATION_OPTIMIZATION",
        "DATE_FORMAT",
        "DATEFIRST",
        "DATEFORMAT",
        "DAYS",
        "DB_CHAINING",
        "DBREINDEX",
        "DEADLOCK_PRIORITY",
        "DECRYPTION",
        "DEFAULT_DATABASE",
        "DEFAULT_FULLTEXT_LANGUAGE",
        "DEFAULT_LANGUAGE",
        "DEFAULT_SCHEMA",
        "DELAY",
        "DELAYED_DURABILITY",
        "DELETED",
        "DELIMITEDTEXT",
        "DELTA",
        "DENSE_RANK",
        "DENSITY_VECTOR",
        "DIRECTORY_NAME",
        "DISABLE",
        "DISABLED",
        "DISABLE_BROKER",
        "DISTRIBUTION",
        "DOCUMENT",
        "DROP_EXISTING",
        "DROPCLEANBUFFERS",
        "DURABILITY",
        "EDGE",
        "EDITION",
        "ELASTIC_POOL",
        "ELEMENTS",
        "ENABLE_BROKER",
        "ENCODING",
        "ENCRYPTED",
        "ENCRYPTION_TYPE",
        "ENCRYPTION",
        "ERROR_BROKER_CONVERSATIONS",
        "ERRORFILE_CREDENTIAL",
        "ERRORFILE_DATA_SOURCE",
        "ERRORFILE_LOCATION",
        "ERRORFILE",
        "ESCAPE_CHAR",
        "ESTIMATEONLY",
        "EXPAND",
        "EXPLAIN",
        "EXPLICIT",
        "EXTENDED_LOGICAL_CHECKS",
        "EXTERNALPUSHDOWN",
        "FAILOVER",
        "FAST",
        "FAST_FORWARD",
        "FIELD_TERMINATOR",
        "FIELDQUOTE",
        "FIELDTERMINATOR",
        "FILE_FORMAT",
        "FILE_TYPE",
        "FILEGROUP",
        "FILEGROWTH",
        "FILENAME",
        "FILESTREAM_ON",
        "FILESTREAM",
        "FILETABLE_COLLATE_FILENAME",
        "FILETABLE_DIRECTORY",
        "FILETABLE_FULLPATH_UNIQUE_CONSTRAINT_NAME",
        "FILETABLE_PRIMARY_KEY_CONSTRAINT_NAME",
        "FILETABLE_STREAMID_UNIQUE_CONSTRAINT_NAME",
        "FILTER_COLUMN",
        "FILTER_PREDICATE",
        "FIPS_FLAGGER",
        "FIRE_TRIGGERS",
        "FIRST_ROW",
        "FIRSTROW",
        "FLUSHAUTHCACHE",
        "FMTONLY",
        "FOLLOWING",
        "FORCE",
        "FORCED",
        "FORCE_FAILOVER_ALLOW_DATA_LOSS",
        "FORCEPLAN",
        "FORCESCAN",
        "FORCESEEK",
        "FORMAT_OPTIONS",
        "FORMAT_TYPE",
        "FORMAT",
        "FORMATFILE_DATA_SOURCE",
        "FORMATFILE",
        "FORWARD_ONLY",
        "FREEPROCCACHE",
        "FREESESSIONCACHE",
        "FREESYSTEMCACHE",
        "FULLSCAN",
        "FULLTEXT",
        "GB",
        "GENERATED",
        "GEO",
        "GEOZONE",
        "HASH",
        "HEAP",
        "HEADER_ROW",
        "HELP",
        "HIDDEN",
        "HIGH",
        "HINT",
        "HISTOGRAM",
        "HISTORY_RETENTION_PERIOD",
        "HISTORY_TABLE",
        "HONOR_BROKER_PRIORITY",
        "IGNORE_CONSTRAINTS",
        "IGNORE_DUP_KEY",
        "IGNORE_NONCLUSTERED_COLUMNSTORE_INDEX",
        "IGNORE_TRIGGERS",
        "IMPERSONATE",
        "IMPLICIT_TRANSACTIONS",
        "INBOUND",
        "INCLUDE_NULL_VALUES",
        "INCLUDE",
        "INCREMENT",
        "INCREMENTAL",
        "INDEXDEFRAG",
        "INFINITE",
        "INLINE",
        "INPUTBUFFER",
        "INSENSITIVE",
        "INSERTED",
        "INSTEAD",
        "IO",
        "ISJSON",
        "JSON",
        "JSON_ARRAY",
        "JSON_ARRAYAGG",
        "JSON_MODIFY",
        "JSON_OBJECT",
        "JSON_OBJECTAGG",
        "JSON_PATH_EXISTS",
        "JSON_QUERY",
        "JSON_VALUE",
        "KB",
        "KEEP",
        "KEEPDEFAULTS",
        "KEEPFIXED",
        "KEEPIDENTITY",
        "KEEPNULLS",
        "KEYSET",
        "KILOBYTES_PER_BATCH",
        "LABEL",
        "LASTROW",
        "LEDGER_VIEW",
        "LEDGER",
        "LEGACY_CARDINALITY_ESTIMATION",
        "LIST",
        "LOB_COMPACTION",
        "LOCATION",
        "LOCK_TIMEOUT",
        "LOG",
        "LOGIN",
        "LOOP",
        "LOW",
        "MANUAL",
        "MANUAL_CUTOVER",
        "MASKED",
        "MASTER",
        "MATCHED",
        "MAX_DURATION",
        "MAX_GRANT_PERCENT",
        "MAX",
        "MARK_IN_USE_FOR_REMOVAL",
        "MAXDOP",
        "MAXERRORS",
        "MAXRECURSION",
        "MAXSIZE",
        "MAXTRANSFERSIZE",
        "MAXVALUE",
        "MB",
        "MEMORY_OPTIMIZED",
        "MEMORY_OPTIMIZED_DATA",
        "MIGRATION_STATE",
        "MIN_GRANT_PERCENT",
        "MINUTES",
        "MINVALUE",
        "MODEL",
        "MOVE",
        "MONTHS",
        "MULTI_USER",
        "MEDIANAME",
        "MEDIAPASSWORD",
        "MUST_CHANGE",
        "NAME",
        "NATIVE_COMPILATION",
        "NESTED_TRIGGERS",
        "NEWNAME",
        "NEW_BROKER",
        "NO_INFOMSGS",
        "NO_QUERYSTORE",
        "NO_STATISTICS",
        "NO_PERFORMANCE_SPOOL",
        "NO_CHECKSUM",
        "NO_TRUNCATE",
        "NO_WAIT",
        "NOCOUNT",
        "NORECOVERY",
        "NOREWIND",
        "NOUNLOAD",
        "NODE",
        "NOEXEC",
        "NOEXPAND",
        "NOLOCK",
        "NON_TRANSACTED_ACCESS",
        "NORMAL",
        "NOWAIT",
        "NTILE",
        "NULLS",
        "NUMERIC_ROUNDABORT",
        "OFFSET",
        "OLD_PASSWORD",
        "ONLINE",
        "OPENJSON",
        "OPENTRAN",
        "OPERATION_TYPE_COLUMN_NAME",
        "OPERATION_TYPE_DESC_COLUMN_NAME",
        "OPTIMISTIC",
        "OPTIMIZE_FOR_SEQUENTIAL_KEY",
        "OPTIMIZE",
        "ORC",
        "OUTBOUND",
        "OUTPUTBUFFER",
        "OVERRIDE",
        "OWNER",
        "PAD_INDEX",
        "PAGE",
        "PAGE_VERIFY",
        "PAGLOCK",
        "PARAMETERIZATION",
        "PARQUET",
        "PARSEONLY",
        "PARSER_VERSION",
        "PARTITIONS",
        "PASSWORD",
        "PAUSE",
        "PAUSED",
        "PERCENTAGE",
        "PERFORM_CUTOVER",
        "PERIOD",
        "PERSISTED",
        "PERSISTENT_LOG_BUFFER",
        "PERSISTENT_VERSION_STORE_FILEGROUP",
        "PHYSICAL_ONLY",
        "POLICY",
        "POPULATION",
        "PRECEDING",
        "PREDICATE",
        "PROCCACHE",
        "PREDICT",
        "PROFILE",
        "PROPERTY",
        "PROVIDER",
        "PUSHDOWN",
        "QUERY_GOVERNOR_COST_LIMIT",
        "QUERY_STORE",
        "QUERYTRACEON",
        "QUOTED_IDENTIFIER",
        "R",
        "RANDOMIZED",
        "RANK",
        "RAW",
        "RCFILE",
        "READCOMMITTED",
        "READCOMMITTEDLOCK",
        "READONLY",
        "READPAST",
        "READ_COMMITTED_SNAPSHOT",
        "READ_ONLY",
        "READ_WRITE",
        "READ_WRITE_FILEGROUPS",
        "READUNCOMMITTED",
        "REBUILD",
        "RECEIVE",
        "RECOMPILE",
        "RESTART",
        "RECOVERY",
        "RECURSIVE_TRIGGERS",
        "REGENERATE",
        "REJECT_SAMPLE_VALUE",
        "REJECT_TYPE",
        "REJECT_VALUE",
        "REJECTED_ROW_LOCATION",
        "REMOTE_DATA_ARCHIVE",
        "REMOTE_PROC_TRANSACTIONS",
        "REMOVE",
        "RENAME",
        "REORGANIZE",
        "REPEATABLE",
        "REPEATABLEREAD",
        "REPLACE",
        "REPLICATE",
        "RESAMPLE",
        "RESEED",
        "REWIND",
        "RESPECT",
        "RESULT_SET_CACHING",
        "RESUMABLE",
        "RESTRICTED_USER",
        "RESUME",
        "RETENTION_PERIOD",
        "ROBUST",
        "ROOT",
        "ROUND_ROBIN",
        "ROW_NUMBER",
        "ROWLOCK",
        "ROWS_PER_BATCH",
        "ROWSET_OPTIONS",
        "ROWTERMINATOR",
        "S",
        "SCALAR",
        "SCALEOUTEXECUTION",
        "SCHEMA_AND_DATA",
        "SCHEMA_ONLY",
        "SCHEMABINDING",
        "SCHEME",
        "SCOPED",
        "SCROLL_LOCKS",
        "SECONDARY",
        "SECONDS",
        "SECRET",
        "SECURITY",
        "SELF",
        "SEND",
        "SEQUENCE_NUMBER_COLUMN_NAME",
        "SEQUENCE_NUMBER",
        "SERDE_METHOD",
        "SERIALIZABLE",
        "SERVER",
        "SERVICE",
        "SERVICEBROKER",
        "SERVICE_OBJECTIVE",
        "SETERROR",
        "SETVAR",
        "SHOWPLAN",
        "SHOW_STATISTICS",
        "SHOWCONTIG",
        "SHOWPLAN_ALL",
        "SHOWPLAN_TEXT",
        "SHOWPLAN_XML",
        "SHRINKDATABASE",
        "SHRINKFILE",
        "SID",
        "SIMPLE",
        "SINGLE_BLOB",
        "SINGLE_CLOB",
        "SINGLE_NCLOB",
        "SNAPSHOT",
        "SORT_IN_TEMPDB",
        "SOURCE",
        "SPARSE",
        "SPATIAL_WINDOW_MAX_CELLS",
        "SPLIT",
        "STANDBY",
        "SQLPERF",
        "STAT_HEADER",
        "STATS",
        "STATISTICAL_SEMANTICS",
        "STATISTICS_INCREMENTAL",
        "STATISTICS_NORECOMPUTE",
        "STOPLIST",
        "STOP_ON_ERROR",
        "STRING_AGG",
        "STRING_DELIMITER",
        "SUBSCRIBE",
        "SWITCH",
        "SYNONYM",
        "SYSTEM_TIME",
        "SYSTEM_VERSIONING",
        "TB",
        "TABLERESULTS",
        "TABLOCK",
        "TABLOCKX",
        "TAPE",
        "TAKE",
        "TARGET",
        "TARGET_RECOVERY_TIME",
        "TEXTIMAGE_ON",
        "THROW",
        "TIES",
        "TIMEOUT",
        "TRACEOFF",
        "TRACEON",
        "TRACESTATUS",
        "TRANSACTION_ID_COLUMN_NAME",
        "TRANSACTION_ID",
        "TRANSFORM_NOISE_WORDS",
        "TRUNCATE_TARGET",
        "TRUSTWORTHY",
        "TRY",
        "TYPE",
        "TYPE_WARNING",
        "TWO_DIGIT_YEAR_CUTOFF",
        "UNBOUNDED",
        "UNCOMMITTED",
        "UNDEFINED",
        "UNLIMITED",
        "UNMASK",
        "UNLOAD",
        "UNSAFE",
        "UPDATEUSAGE",
        "UPDLOCK",
        "URL",
        "USE_TYPE_DEFAULT",
        "USED",
        "USER_DB",
        "USEROPTIONS",
        "VIEW_METADATA",
        "WAIT_AT_LOW_PRIORITY",
        "WEEK",
        "WEEKS",
        "WINDOWS",
        "WITHOUT_ARRAY_WRAPPER",
        "XACT_ABORT",
        "XLOCK",
        "XML_COMPRESSION",
        "XML",
        "XMLDATA",
        "XMLSCHEMA",
        "XSINIL",
        "YEARS",
        // Data type keywords
        // https://learn.microsoft.com/en-us/sql/t-sql/data-types/data-types-transact-sql
        "TINYINT",
        "BIGINT",
        "MONEY",
        "SMALLMONEY",
        "SMALLDATETIME",
        "DATETIME",
        "DATETIME2",
        "DATETIMEOFFSET",
        "TEXT",
        "NVARCHAR",
        "NTEXT",
        "VARBINARY",
        "IMAGE",
        "SQL_VARIANT",
        "ROWVERSION",
        "UNIQUEIDENTIFIER",
        "GEOGRAPHY",
        "GEOMETRY",
        "HIERARCHYID",
        "VECTOR",
        "VERIFY_CLONEDB",
    ]
    .into_iter()
    .collect()
}

/// Case-sensitive method names exposed by SQL Server data types.
pub(crate) fn tsql_datatype_methods() -> HashSet<&'static str> {
    [
        // XML methods.
        "exist",
        "modify",
        "nodes",
        "query",
        "value",
        // hierarchyid methods.
        "GetAncestor",
        "GetLevel",
        "GetReparentedValue",
        "GetRoot",
        "IsDescendantOf",
        "Parse",
        "ToString",
        // Geometry and geography methods.
        "STArea",
        "STLength",
        "STBoundary",
        "STCentroid",
        "STEnvelope",
        "STConvexHull",
        "STAsBinary",
        "STAsText",
        "STGeometryType",
        "STDimension",
        "STSrid",
        "STStartPoint",
        "STEndPoint",
        "STPointN",
        "STPointOnSurface",
        "STCurveN",
        "STCurveToLine",
        "STX",
        "STY",
        "STExteriorRing",
        "STInteriorRingN",
        "STNumInteriorRing",
        "STGeometryN",
        "STNumGeometries",
        "STNumPoints",
        "STNumCurves",
        "STContains",
        "STCrosses",
        "STDisjoint",
        "STEquals",
        "STIntersects",
        "STOverlaps",
        "STRelate",
        "STTouches",
        "STWithin",
        "STDistance",
        "STBuffer",
        "STDifference",
        "STIntersection",
        "STSymDifference",
        "STUnion",
        "STIsClosed",
        "STIsEmpty",
        "STIsRing",
        "STIsSimple",
        "STIsValid",
    ]
    .into_iter()
    .collect()
}

/// T-SQL parser references retained outside the keyword classification sets.
pub(crate) fn tsql_additional_parser_keywords() -> HashSet<&'static str> {
    [
        "APPEND",
        "APPLICATION",
        "AVAILABILITY",
        "BASE64",
        "CLASSIFICATION",
        "CONFIGURATION",
        "CONTRACT",
        "DATASPACE",
        "DDL",
        "DEFINITION",
        "DMY",
        "DYM",
        "ENDPOINT",
        "ERROR_LINE",
        "ERROR_MESSAGE",
        "ERROR_NUMBER",
        "ERROR_PROCEDURE",
        "ERROR_SEVERITY",
        "ERROR_STATE",
        "EVENT",
        "LIBRARY",
        "LINKED",
        "MASK",
        "MDY",
        "MESSAGE",
        "MYD",
        "NEWID",
        "NOTIFICATION",
        "NOTIFICATIONS",
        "OBJECT_ID",
        "OPERATIONS",
        "QUERY",
        "QUEUE",
        "REMOTE",
        "RESOURCES",
        "ROUTE",
        "SCRIPT",
        "SECURABLES",
        "SENSITIVITY",
        "SETTINGS",
        "TRACE",
        "TRACKING",
        "YDM",
        "YMD",
    ]
    .into_iter()
    .collect()
}
