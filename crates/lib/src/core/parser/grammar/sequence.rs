    ...
    fn match_segments(
        ...
        for elem in &self.elements {
            if let Some(indent) = elem.as_any().downcast_ref::<Conditional>() {
                ...
                for (_, submatch) in match_result.insert_segments {
                    meta_buffer.push(submatch.as_any().downcast_ref::<Indent>().unwrap().clone());
                }
                continue;
            ...
            if !elem_match.has_match() {
                ...
                start_idx = skip_start_index_forward_to_code(segments, start_idx, max_idx);
                return unimplemented!();
            }
        ...
        if matches!(self.parse_mode, ParseMode::Greedy | ParseMode::GreedyOnceStarted) {
            if max_idx > matched_idx {
                idx = skip_start_index_forward_to_code(segments, matched_idx, max_idx);
