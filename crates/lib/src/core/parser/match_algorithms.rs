    ...
    loop {
        ...
        if end_brackets.iter().any(|it| it.dyn_eq(matcher.as_ref())) {
            ...
            if closing_idx == type_idx {
                ...
                return Ok(match_result.wrap(Matched::ErasedSegment(
                    BracketedSegment::new(
                        vec![],
                        vec![segments[opening_match.span.start as usize].clone()],
                        vec![segments[match_span.start as usize].clone()],
                        true,
                    )
                    .to_erased_segment(),
                )));
...
pub fn greedy_match(
    ...
    loop {
        ...
        let start_idx = match_result.span.start;
        let stop_idx = match_result.span.end;
...
pub fn trim_to_terminator(
    ...
    let early_return = parse_context.deeper_match(false, &[], |ctx| {
        ...
        for term in pruned_terms {
            if term
                .match_segments(segments, idx, ctx)
