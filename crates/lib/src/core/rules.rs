...
use crate::core::rules::config::RuleConfigOption;
...
pub trait Rule: Debug + 'static + Send + Sync {
    ...
    /// The typed configuration options this rule accepts.
    ///
    /// Rules with configuration return `RuleXXConfig::config_options()`, which
    /// is generated by [`rule_config!`](crate::rule_config). It is what
    /// `docs/reference/rules.md` is generated from.
...
impl RuleSet {
    ...
    pub(crate) fn get_rulepack(&self, config: &FluffConfig) -> Result<RulePack, SQLFluffUserError> {
        ...
        // Values set directly under `[sqruff:rules]` are shared defaults for every
        // rule that accepts them; a rule's own section takes precedence.
        let shared_config: HashMap<String, Value> = rules
            .iter()
            .filter(|(_, value)| value.as_map().is_none())
        ...
        for code in keylist {
            ...
            let mut rule_config = shared_config.clone();
            if let Some(section) = rules.get(rule_config_ref).and_then(Value::as_map) {
                for (key, value) in section {
                    rule_config.insert(key.clone(), value.clone());
                }
            }

            instantiated_rules.push(rule.load_from_config(&rule_config).map_err(|err| {
                SQLFluffUserError::new(format!(
                    "Error in configuration for rule {} ({}): {err}",
                    rule.code(),
                    rule.name()
                ))
            })?);
