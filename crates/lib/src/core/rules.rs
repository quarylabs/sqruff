...
pub trait Rule: Debug + 'static + Send + Sync {
    ...
    /// The configuration options this rule accepts, as
    /// `(name, description, default)`. Rules with a typed configuration return
    /// the options generated by
    /// [`typed_config!`](sqruff_lib_core::typed_config); the rest have nothing
    /// to configure.
