...
fn eval(context: &RuleContext) -> Vec<LintResult> {
    let rule = RuleLT09 {
        wildcard_policy: context.config.raw["layout"]["select_targets"]["wildcard_policy"]
            .as_string()
            .unwrap_or("single")
            .to_owned(),
    };
    
    let select_targets_info = get_indexes(context);
    ...
    if select_targets_info.select_targets.len() == 1
        && (!has_wildcard || rule.wildcard_policy == "single")
    {
        return eval_single_select_target_element(select_targets_info, context);
    } else if !select_targets_info.select_targets.is_empty() {
        return eval_multiple_select_target_elements(
            context.tables,
            select_targets_info,
...
fn load_from_config(
    ...
    Ok(RuleLT09 {
        wildcard_policy: config.get("wildcard_policy")
