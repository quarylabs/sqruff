...
col2_value = test_value
        ...
        let test_cases = [
            ...
            (
                r#"SELECT col1 from my_table WHERE col2 = "${col2_value}""#,
                r#"SELECT col1 from my_table WHERE col2 = "test_value""#,
            ),
            (
                r#"SELECT col1 from my_table WHERE col2 = '$col2_value'"#,
                r#"SELECT col1 from my_table WHERE col2 = 'test_value'"#,
        ...
        for (input, expected) in test_cases {
            let out_str = templater
                .process(input, "test.sql", &config, &None)
                .unwrap();
            assert_eq!(expected, out_str.templated());
        }
    }
}
