...
class JinjaTemplater(PythonTemplater):
    ...
    def _get_jinja_env(self, config: Optional[FluffConfig] = None) -> Environment:
        ...
        ignore_templating = config and config.jinja_ignore_templating
    ...
    def _get_loader_search_path(
        ...
        """
        if config:
            loader_search_path = config.jinja_loader_search_path
            if loader_search_path:
                return loader_search_path
        return None
    ...
    def _apply_dbt_builtins(self, config: Optional[FluffConfig]) -> bool:
        ...
        """
        if config:
            return config.jinja_apply_dbt_builtins
        return False
    ...
    def _get_env_context(
        ...
        """
        # Load the context
        live_context = self.get_context(fname, config, context)
    ...
    def construct_render_func(
        ...
        live_context = self._get_env_context(fname, config, context, env)
    ...
    def process(
        ...
        *,
        ...
        undefined_variables = self._init_undefined_tracking(
            ...
            ignore_templating=config.jinja_ignore_templating if config else False,
        )
        ...
        try:
            ...
            raise SQLTemplaterError(
                (
                    f"Unrecoverable failure in Jinja templating: {err}. Have you "
