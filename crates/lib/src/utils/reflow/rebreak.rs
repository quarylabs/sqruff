...
impl RebreakIndices {
    fn from_elements(elements: &ReflowSequenceType, start_idx: usize, dir: i32) -> Option<Self> {
        ...
        {
            let neighbor = newline_point_idx + dir as isize;
            if neighbor < 0 || neighbor >= elements.len() as isize {
                break;
            }
            if elements[newline_point_idx as usize]
                .class_types()
                .contains(SyntaxKind::Newline)
                || elements[neighbor as usize]
                    .segments()
                    .iter()
                    .any(|seg| seg.is_code())
            {
        ...
        {
            let neighbor = pre_code_point_idx + dir as isize;
            if neighbor < 0 || neighbor >= elements.len() as isize {
                break;
            }
            if elements[neighbor as usize]
                .segments()
                .iter()
                .any(|seg| seg.is_code())
            {
