...
mod tests {
    ...
    fn test_lineage_lateral_flatten() {
        let dialect = sqruff_lib_dialects::snowflake::dialect(None).unwrap();
        let parser = Parser::new(&dialect, Default::default());
    ...
    #[test]
    fn test_subquery() {
        let dialect = sqruff_lib_dialects::ansi::dialect(None).unwrap();
        let parser = Parser::new(&dialect, Default::default());
    ...
    fn test_lineage_cte_union() {
        ...
        SELECT x, created_at FROM dataset;",
            ...
            assert_eq!(
                downstream_a.reference_node_name, "dataset",
                "Failed for dialect: {}",
                dialect_name
            );
    ...
    fn test_node_name_doesnt_contain_comment() {
        for (dialect_name, dialect) in all_dialects() {
            ...
            assert_eq!(
                tables.nodes[downstream.downstream[0]].name, "t1.x",
                "Failed for dialect: {}",
                dialect_name
            );
        }
