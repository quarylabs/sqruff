...
/// figure_out_skippable_tests returns a map of the tests that can be skipped as
/// an array with an enum. The enum is either Run or Skip.
///
...
pub fn figure_out_skippable_tests(
    ...
    let all_inferred_tests = sql_map
        .iter()
        .flat_map(|(path, sql)| {
            if let Ok(tests) = infer_tests(parser, path, sql, tests) {
    ...
    fn test_figure_out_skippable_steps_count_star() {
        ...
        let tests = vec![
            ...
            TestStructure {
                ...
                sql: HashMap::from([(
                    "q.model_child".to_string(),
                    "WITH test AS (SELECT count(*) AS counter FROM q.model_parent) SELECT counter \
                     from test"
                        .to_string(),
                )]),
                input_tests: vec![Test::GreaterThanOrEqual(ComparisonTest {
